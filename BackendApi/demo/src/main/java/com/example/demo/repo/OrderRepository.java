...
public interface OrderRepository extends JpaRepository<Orders,Long> {
    ...
    @Query("SELECT   Day(createdAt), sum(price),month(createdAt),year(createdAt) from Orders where MONTH(createdAt) = ?1 and order_process = 3 group by day(createdAt),year(createdAt) ")
